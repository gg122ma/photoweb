...
function normalizeShoot(s){
    ...
    // galleryImages 优先用传入的值，绝不用封面覆盖
    const gallery=parseGalleryImages(s.galleryImages||s.gallery_images||s.images);
    return{
        ...
        cover:s.cover||s.cover_url||DEFAULT_COVER,
...
function renderHome(){
    ...
    const slideHTML=slides.map((s,i)=>'<div class="slider-slide'+(i===0?' active':'')+'\" data-index="'+i+'">'
        +'<img class="slide-bg" src="'+s.image_url+'" alt="'+s.caption+'" draggable="false">'
        +'<div class="slider-overlay"></div>'
        +'<div class="slider-content">'
        +'<div class="slider-index">NO.'+String(i+1).padStart(2,'0')+' / '+String(total).padStart(2,'0')+'</div>'
        +'<h1 class="slider-title">'+s.caption+'</h1>'
        +'<div class="slider-divider"></div>'
        +'<p class="slider-meta">'+s.sub+'</p>'
        +'</div>'
        +'<div class="slider-counter-box">'
        +'<span class="slider-counter-cur">'+String(i+1).padStart(2,'0')+'</span>'
        +'<span class="slider-counter-sep">—</span>'
        +'<span class="slider-counter-total">'+String(total).padStart(2,'0')+'</span>'
        +'</div>'
        +'</div>').join('');
...
document.getElementById('naChangeCover').addEventListener('click',()=>{
    ...
    openCldUpload(async url=>{SHOOTS=SHOOTS.map(s=>s.slug===naCurrentSlug?{...s,cover:url}:s);const updated=shootBySlug(naCurrentSlug);if(updated)await saveShootToSupabase(updated);document.getElementById('naDetailCoverImg').src=url;await refreshSite();renderNaAlbumList()});
});
